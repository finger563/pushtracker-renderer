...
void Poly::SetupRasterization( ) {
	...
	for (int u=0;u<POLY_MAX_VERTICES;u++) {
		for (int i=0;i<NUM_VERTEX_DATA;i++) {
			increments[u][0][i] = vl[i];
			increments[u][1][i] = vr[i];
		}
	}
...
void Poly::Rasterize( float* zbuff, char* dispbuff ) {
	for ( int y = MaxY(); y >= MinY(); y--) {
		...
		for (int x=ceil(vi.x);x<=floor(ev.x);x++) {
			...
			vi = sv + (ev-sv)*ai;
			if ( vi.ez/vi.hw < zbuff[x + y*SIZE_X] ) {
				zbuff[x + y*SIZE_X] = vi.ez/vi.hw;
				switch ( rType ) {	// What are we interpolating/rendering?
				case FLAT:
					dispbuff[x + y*SIZE_X] = RGB_MAKE((int)(r*255),(int)(g*255),(int)(b*255));
					break;
				case COLORED:
					...
					dispbuff[x + y*SIZE_X] = RGB_MAKE((int)(vi.r*255),(int)(vi.g*255),(int)(vi.g*255));
					break;
				case SMOOTH:
					break;
				case TEXTURED:
					...
					if ( vi.v < 0 || vi.v > 1 )
						vi.v = 0;
					dispbuff[x + y*SIZE_X] = texture[(int)(vi.u*(texwidth-1)) + ((int)(vi.v*(texheight-1)))*texwidth];
					break;
				case TEXTURED_SMOOTH:
					break;
				default:
...
void Poly::Rasterize( const int y, float* zbuff, char* dispbuff ) {
	if ( y > MaxY() || y < MinY() )
		return;
	float BC[POLY_MAX_VERTICES] = {};		// boundary tests against y scanline
	int line[POLY_MAX_VERTICES] = {};	// which lines cross y scanline
	...
	for (int x=vi.x;x<=ev.x;x++) {
		...
		if ( vi.ez/vi.hw < zbuff[x + y*SIZE_X] ) {
			...
			switch ( rType ) {	// What are we interpolating/rendering?
			case FLAT:
				dispbuff[x + y*SIZE_X] = RGB_MAKE((char)(r*255.0),(char)(g*255.0),(char)(b*255.0));
				break;
			case COLORED:
				vi.r = vi.r/vi.hw;	// divide all interpolated values by hw 
				vi.g = vi.g/vi.hw;	// divide all interpolated values by hw 
				vi.b = vi.b/vi.hw;	// divide all interpolated values by hw 
				dispbuff[x + y*SIZE_X] = RGB_MAKE((char)(vi.r*255.0),(char)(vi.g*255.0),(char)(vi.b*255.0));
				break;
			case SMOOTH:
				break;
			case TEXTURED:
...
void Poly::RasterizeFast( float* zbuff, char* dispbuff ) {
}

void Poly::RasterizeFast( const int y, float* zbuff, char* dispbuff ) {
	...
	for (int x=sx;x<=ex;x++) {
		...
		if ( px[6]/px[NUM_VERTEX_DATA-1] < zbuff[x] ) {
			zbuff[x] = px[6]/px[NUM_VERTEX_DATA-1];
			switch ( rType ) {
			default:
			case FLAT:
				dispbuff[x] = RGB_MAKE((char)(r*255.0),(char)(g*255.0),(char)(b*255.0));
				break;
			case COLORED:
				...
				dispbuff[x] = RGB_MAKE((char)(px[9]*255.0),(char)(px[10]*255.0),(char)(px[11]*255.0));
				break;
			case SMOOTH:
				break;
			case TEXTURED:
				...
				if ( px[8] < 0 || px[8] > 1 )
					px[8] = 0;
				dispbuff[x] = texture[(int)(px[7]*(texwidth-1)) + ((int)(px[8]*(texheight-1)))*texwidth];
				break;
			case TEXTURED_SMOOTH:
				break;
			}
